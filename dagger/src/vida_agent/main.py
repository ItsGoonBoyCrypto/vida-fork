...
FRAME STATUS: this module is scaffolding. The function surface and the
security boundary are the design; run `dagger develop` to materialize the
SDK, then iterate. TODOs are marked inline.
"""
...
class VidaAgent:
    ...
    # ── LLM binding (the "agents can use it freely" part) ─────────────────
    ...
    def agent_env(
        ...
        """
        An LLM environment with this wallet bound as a tool surface.

        Dagger exposes every function of the bound module to the model, so an
        agent can describe/balance/send within session limits — and nothing
        else. Compose it like:

            dag.llm().with_env(vida.agent_env(...)).with_prompt("...")

        TODO(frame): once `dagger develop` has generated bindings, verify the
        Env input wiring against the installed engine version — the Env API
        surface has evolved across 0.17 → 0.18 releases.
        """
        return (
            dag.env()
            .with_string_input(
                "instructions",
                "You are operating a Vida agent wallet on Kaspa. "
                "Call vida_describe first to learn your session limits. "
                "Never retry a policy rejection; report it to the owner.",
                "Standing instructions for the wallet-holding agent",
            )
            # TODO(frame): bind self (with wallet/session applied) as a typed
            # module input so the LLM can call describe/balance/send directly.
        )
